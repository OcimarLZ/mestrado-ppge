...
def processar_ies_sigla(engine, sigla):
    sql = """
    SELECT distinct
    i.nome as ies,
    cc.ano_censo, 
...
def preencher_aba_ies(ws, dados_ies):
    ws.title = dados_ies['sigla'][:31] if len(dados_ies['sigla']) > 31 else dados_ies['sigla']
...
def main():
    ...
    print("Obtendo lista de IES...")
    with engine.connect() as conn:
        siglas_result = conn.execute(text("""
            SELECT DISTINCT i.sigla, i.nome, i.codigo
            FROM ies i 
            JOIN curso_censo cc ON i.codigo = cc.ies
            WHERE i.categoria = 1 AND i.org_academica = 1 AND i.sigla <> '-'
            ORDER BY i.sigla
        """)).fetchall()
    print(f"Total de IES encontradas: {len(siglas_result)}")

    wb = Workbook()
    ws_default = wb.active
    wb.remove(ws_default)

    for idx, (sigla, nome, codigo) in enumerate(siglas_result, start=1):
        print(f"[{idx}/{len(siglas_result)}] Processando {sigla} ({nome})...")
        dados_ies = processar_ies_sigla(engine, sigla)
        if dados_ies:
            print(f"  -> Dados encontrados para {sigla}")
            ws = wb.create_sheet()
            preencher_aba_ies(ws, dados_ies)
        else:
            print(f"  -> Nenhum dado encontrado para {sigla}")

    arquivo_saida = Path(__file__).parent / "resumo_todas_ies.xlsx"
