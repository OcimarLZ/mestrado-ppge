...
# SQL parametrizado para recuperar municipios com licenciaturas presenciais em universidades federais.
# A coluna municipio do curso_censo guarda o codigo IBGE.
# tp_grau_academico = 2 corresponde a licenciatura.
# org_academica = 1 corresponde a universidade.
# categoria = 1 corresponde a federal.
# tp_modalidade_ensino = 1 corresponde a presencial.
sql = """
SELECT DISTINCT
    cc.ano_censo,
    cc.municipio AS code_muni
FROM curso_censo cc
WHERE cc.ano_censo = :ano
...
def buscar_dados_por_ano(ano: int) -> pd.DataFrame:
    ...
    df_ano = normalizar_codigos_municipio(df_ano)
    df_ano['total_registros'] = 1
    return df_ano
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Municipios com Licenciaturas Presenciais em Universidades Federais: {ano_ini} x {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (licenciaturas presenciais em universidades federais)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_lic_2periodos_ufs_presencial_{anos_str}.png'

    gerar_mapa_combinado_a4(anos_alvo, dados_por_ano, malha_municipios, output_path)
    print(f'Mapa comparativo gerado: {output_path}')
