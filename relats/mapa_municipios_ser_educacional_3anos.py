...
onde há cursos vinculados às mantenedoras da Ser Educacional.

Municípios sem presença ficam em cinza claro; os presentes ficam em
verde fixo, independente do número de cursos.
"""
...
MANTENEDORAS = (206, 332, 525, 815, 1262, 1847, 2405, 2447, 2587, 16727, 18310, 18494)
...
COR_PRESENTE = '#1A7A1A'  # verde da Ser Educacional
...
def gerar_mapa_ano(
    ...
    ax.set_title(
        f'Presença da Ser Educacional no Ensino Superior — {ano}\n'
    ...
    legenda_patches = [
        ...
        mpatches.Patch(color=COR_PRESENTE, label='Com presença (Ser Educacional)'),
    ]
...
def gerar_painel_a4(df: pd.DataFrame, malha: gpd.GeoDataFrame, output_path: str) -> None:
    ...
    fig.suptitle(
        'Expansão territorial da Ser Educacional no Ensino Superior (2005–2014–2024)',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in ANOS:
        df_ano = df[df['ano_censo'] == ano].copy()
        output_path = f'docs/graficos/mapa_ser_educacional_{ano}.png'
        gerar_mapa_ano(ano, df_ano, malha, output_path)

    output_painel = 'docs/graficos/mapa_ser_educacional_painel_a4.png'
    gerar_painel_a4(df, malha, output_painel)

    print('\nTodos os arquivos gerados com sucesso!')
    for ano in ANOS:
        print(f'- docs/graficos/mapa_ser_educacional_{ano}.png')
