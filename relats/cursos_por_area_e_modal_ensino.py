...
# Monta o SQL para incluir a modalidade de ensino
sql = """
SELECT 
    cc.ano_censo AS ano,
    CASE 
...
GROUP BY cc.ano_censo, modalidade_ensino, cc.area_geral
ORDER BY ano, modalidade_ensino, area;
"""
...
# Criando a tabela pivotada com áreas e modalidades de ensino nas linhas e anos nas colunas
df_pivot = df.pivot_table(index=['area', 'modalidade_ensino'], columns='ano', values='qtde_cursos', fill_value=0)

# Configurações para exportação em HTML
years = sorted(df['ano'].unique())
column_html = ['200px', '150px'] + ['100px'] * len(years)
column_names = ['Área', 'Modalidade de Ensino'] + [str(year) for year in years]
...
arq_nome = 'cursos_area_modalidade_ano_qtde'
...
<table style="width: 100%; border-collapse: collapse;">
    <tr style="background-color: #2E7D32;">
        <th colspan="{len(df_pivot.columns) + 2}" style="font-size: 14px; font-family: Tahoma, sans-serif; color: white; padding: 10px; text-align: center;">
            Ensino Superior - Nível Graduação: Evolução do número de cursos ofertados em Chapecó por Área de Conhecimento e Modalidade de Ensino
        </th>
    </tr>
</table>
"""
...
print('Tabela HTML com separação por área e modalidade de ensino criada com sucesso, sem precisão decimal.')
