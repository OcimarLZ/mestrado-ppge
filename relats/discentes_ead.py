...
arq_nome = 'evolucao_discentes_ead_qtde'
...
arq_output = os.path.join(base_dir, 'static', 'tabelas', 'evolucao_discentes_ead_qtde.html')
...
plt.title('Evolução dos Discentes por Ano em Chapecó (Presencial)', color='#000000')
plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small')  # Ajustando a posição da legenda
plt.grid(True)

# Salvando o gráfico
arq_output = os.path.join(base_dir, 'static', 'graficos', arq_nome + '.png')

# Criar o diretório se não existir
os.makedirs(os.path.dirname(arq_output), exist_ok=True)
