...
onde há cursos vinculados às mantenedoras da Vitru.

Municípios sem presença ficam em cinza claro; os presentes ficam em
roxo escuro fixo, independente do número de cursos.
"""
...
COR_PRESENTE = '#E65C00'  # laranja da Vitru
...
def gerar_mapa_ano(
    ...
    ax.set_title(
        f'Presença da Vitru no Ensino Superior — {ano}\n'
    ...
    legenda_patches = [
        ...
        mpatches.Patch(color=COR_PRESENTE, label='Com presença (Vitru)'),
    ]
...
def gerar_painel_a4(df: pd.DataFrame, malha: gpd.GeoDataFrame, output_path: str) -> None:
    ...
    fig.suptitle(
        'Expansão territorial da Vitru no Ensino Superior (2005–2014–2024)',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in ANOS:
        df_ano = df[df['ano_censo'] == ano].copy()
        output_path = f'docs/graficos/mapa_vitru_{ano}.png'
        gerar_mapa_ano(ano, df_ano, malha, output_path)

    output_painel = 'docs/graficos/mapa_vitru_painel_a4.png'
    gerar_painel_a4(df, malha, output_painel)

    print('\nTodos os arquivos gerados com sucesso!')
    for ano in ANOS:
        print(f'- docs/graficos/mapa_vitru_{ano}.png')
