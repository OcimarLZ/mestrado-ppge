...
WHERE 
    cc.municipio = 4204202 AND cc.tp_modalidade_ensino = 1 AND cc.ano_censo BETWEEN 2014 AND 2023
GROUP BY 
    i.nome
ORDER BY 
    total_cursos DESC
...
JOIN 
    ies i ON cc.ies = i.codigo
WHERE 
    cc.municipio = 4204202 AND cc.tp_modalidade_ensino = 1 
...
arq_nome = 'evolucao_cursos_top_7_presencial'
...
<table style="width: 100%; border-collapse: collapse;">
    <tr style="background-color: #2E7D32;">
        <th colspan="{len(df_pivot.columns) + 1}" style="font-size: 14px; font-family: Tahoma, sans-serif; color: white; padding: 10px; text-align: center;">
            Ensino Superior - Nível Graduação: Evolução dos Cursos Ofertados (Modalidade Presencial) das Top 7 IES em Chapecó (2014-2023)
        </th>
    </tr>
