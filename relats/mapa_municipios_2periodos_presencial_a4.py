...
# SQL parametrizado para recuperar municipios com cursos presenciais por ano.
# A coluna municipio do curso_censo guarda o codigo IBGE.
# tp_modalidade_ensino = 1 corresponde a modalidade presencial.
sql = """
SELECT
    c.ano_censo,
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Comparativo de Municípios com Cursos Presenciais no Censo da Educação Superior: {ano_ini} × {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (presencial)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_2periodos_presencial_{anos_str}_a4.png'

    gerar_mapa_combinado_a4(anos_alvo, dados_por_ano, malha_municipios, output_path)
    print(f'Mapa A4 comparativo (presencial) gerado: {output_path}')
