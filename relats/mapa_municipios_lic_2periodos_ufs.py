...
# SQL parametrizado para recuperar municipios com licenciaturas em universidades federais.
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Municipios com Licenciaturas em Universidades Federais: {ano_ini} x {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (licenciaturas em universidades federais)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_lic_2periodos_ufs_{anos_str}.png'
