...
SELECT 
    ...
    SUM(i.qt_tec_total) AS tecnicos
FROM 
    ies_censo i
WHERE 
    i.municipio = 4204202
...
arq_nome = 'evolucao_colaboradores_ano'
...
sns.lineplot(data=df, x='ano', y='docentes', marker='o', label='Docentes', color='#006400')
sns.lineplot(data=df, x='ano', y='tecnicos', marker='o', label='Técnicos', color='#FF6347')

# Adicionando anotações para cada ponto
for i in range(df.shape[0]):
    plt.annotate(f"{df['docentes'].iloc[i]}", (df['ano'].iloc[i], df['docentes'].iloc[i]), textcoords="offset points", xytext=(0, 10), ha='center', color='#006400')
    plt.annotate(f"{df['tecnicos'].iloc[i]}", (df['ano'].iloc[i], df['técnicos'].iloc[i]), textcoords="offset points", xytext=(0, 10), ha='center', color='#FF6347')

plt.xlabel('Ano')
plt.ylabel('Quantidade de Colaboradores')
plt.title('Evolução dos Colaboradores na Educação Superior por Ano em Chapecó', color='#000000')
