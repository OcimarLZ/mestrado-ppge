...
# Dimensoes em polegadas — ligeiramente maiores que A4 landscape para mais area nos mapas
A4_LANDSCAPE_W = 15.0
A4_LANDSCAPE_H = 10.0
...
def gerar_mapa_combinado_a4(
    ...
    """Gera uma imagem A4 landscape com os 3 mapas lado a lado."""
    ...
    fig.suptitle(
        'Municípios presentes no Censo da Educação Superior por Período',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
    ...
    plt.subplots_adjust(left=0.005, right=0.995, top=0.92, bottom=0.01, wspace=0.02)
...
def main() -> None:
    ...
    output_path = f'docs/graficos/mapa_municipios_periodos_{anos_str}_a4.png'

    gerar_mapa_combinado_a4(anos_alvo, dados_por_ano, malha_municipios, output_path)
    print(f'Mapa A4 combinado gerado: {output_path}')
