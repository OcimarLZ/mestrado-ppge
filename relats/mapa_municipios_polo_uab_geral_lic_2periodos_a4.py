...
# SQL parametrizado para recuperar municipios de polos UAB gerais com licenciaturas
sql = """
SELECT DISTINCT
    uc.ano_censo,
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Municípios com Polos UAB (Geral - Licenciaturas): {ano_ini} x {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (polos UAB geral licenciaturas)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_polo_uab_geral_lic_{anos_str}.png'
