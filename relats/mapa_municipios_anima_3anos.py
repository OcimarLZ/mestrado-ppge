...
onde há cursos vinculados às mantenedoras selecionadas.

Municípios sem presença ficam em cinza claro; os presentes ficam em
azul com intensidade proporcional ao número de cursos no município.
"""
...
MANTENEDORAS = (
    3052, 268, 321, 414, 481, 1117, 2095, 902, 313, 290,
    14298, 15225, 16079, 821, 1077, 1329, 1198, 1913, 325,
    2003, 340, 1090, 475,
)
...
COR_PRESENTE = '#4B0082'  # roxo escuro da Anima
...
def gerar_mapa_ano(
    ...
    # Municípios presentes em roxo escuro fixo
    ...
    ax.set_title(
        f'Presença da Anima no Ensino Superior — {ano}\n'
    ...
    legenda_patches = [
        ...
        mpatches.Patch(color=COR_PRESENTE, label='Com presença (Anima)'),
    ]
...
def gerar_painel_a4(df: pd.DataFrame, malha: gpd.GeoDataFrame, output_path: str) -> None:
    ...
    # A4 paisagem em polegadas: 11.69 × 8.27
    ...
    fig.suptitle(
        'Expansão territorial da Anima no Ensino Superior (2005–2014–2024)',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in ANOS:
        df_ano = df[df['ano_censo'] == ano].copy()
        output_path = f'docs/graficos/mapa_anima_{ano}.png'
        gerar_mapa_ano(ano, df_ano, malha, output_path)

    output_painel = 'docs/graficos/mapa_anima_painel_a4.png'
    gerar_painel_a4(df, malha, output_painel)

    print('\nTodos os arquivos gerados com sucesso!')
    for ano in ANOS:
        print(f'- docs/graficos/mapa_anima_{ano}.png')
