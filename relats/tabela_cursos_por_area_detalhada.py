...
df_pivot = df.pivot(index='area_detalhada', columns='ano', values='qtde_cursos')
...
column_names = ['Área Detalhada'] + [str(year) for year in years]
...
arq_nome = 'cursos_area_detalhada_ano_qtde'
...
<table style="width: 100%; border-collapse: collapse;">
    <tr style="background-color: #2E7D32;">
        <th colspan="{len(df_pivot.columns) + 1}" style="font-size: 14px; font-family: Tahoma, sans-serif; color: white; padding: 10px; text-align: center;">
            Ensino Superior - Nível Graduação: Evolução do número de cursos ofertados em Chapecó por Área Detalhada
        </th>
    </tr>
</table>
"""
...
total_cursos = df.groupby('area_detalhada')['qtde_cursos'].sum().sort_values(ascending=False)
top_15_areas = total_cursos.head(15)

# Filtrando dados das 15 maiores áreas
df_top_areas = df[df['area_detalhada'].isin(top_15_areas.index)]

# Definindo uma paleta de cores personalizada para evitar repetições
colors = sns.color_palette("husl", len(df_top_areas['area_detalhada'].unique()))

plt.figure(figsize=(12, 8), facecolor='#CCFFCC')
ax = plt.gca()
ax.set_facecolor('#F0F0F0')

for idx, area in enumerate(df_top_areas['area_detalhada'].unique()):
    df_area = df_top_areas[df_top_areas['area_detalhada'] == area]
...
plt.title('Evolução do número de cursos ofertados por Área Detalhada (Top 15)', color='#000000')
