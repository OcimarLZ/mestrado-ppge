...
# SQL parametrizado para recuperar municipios de polos UAB com licenciaturas vinculados a IES NAO federais (categoria > 1)
sql = """
SELECT DISTINCT
    uc.ano_censo,
    uc.municipio AS code_muni
FROM uab_censo uc
LEFT JOIN ies i on i.codigo = uc.ies
WHERE uc.ano_censo = :ano
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Municípios com Polos UAB em Outras IES - Não Federais (Licenciaturas): {ano_ini} x {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (polos UAB nao federais licenciaturas)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_polo_uab_nao_federais_lic_{anos_str}.png'
