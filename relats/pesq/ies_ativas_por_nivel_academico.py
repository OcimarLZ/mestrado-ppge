...
from curso_censo c
join tp_categoria_administrativa ct on ct.codigo = c.categoria
group by c.ano_censo, ct.nome
order by c.ano_censo, ct.nome
"""
df = carregar_dataframe(sql)

# Pivotando para ter colunas por categoria
pivot_df = df.pivot_table(index='ano', columns='categoria', values='QtdeIes', fill_value=0).astype(int)
...
arq_nome = 'ies_ativas_por_categoria'
...
for idx, categoria in enumerate(categorias):
    ax.plot(anos, pivot_df[categoria], marker='o', label=categoria, color=colors[idx % len(colors)], linewidth=2)
    for i, v in enumerate(pivot_df[categoria]):
        if v > 0:
...
ax.set_title('IES Ativas por Ano e Categoria Administrativa', color='#000000', fontsize=14)
ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.25), ncol=3, fontsize='medium', title='Categoria', frameon=False)
