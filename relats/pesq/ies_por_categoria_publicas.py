...
# Monta o SQL (eliminando categorias 4 e 6)
sql = """
select
i.ano_censo as ano,
...
# Calculando o total por ano (mantido para a tabela HTML)
...
arq_nome = 'ies_por_categoria_publica'
...
<table style="width: 100%; border-collapse: collapse;">
    <tr style="background-color: #2E7D32;">
        <th colspan="{len(colunas_html)}" style="font-size: 14px; font-family: Tahoma, sans-serif; color: white; padding: 10px; text-align: center;">
            Quantidade de IES Ativas por Ano e Categoria Administrativa (Exceto Categorias 4 e 6)
        </th>
    </tr>
</table>
"""
...
cores_categorias = {
    'Especial': '#DA70D6',  # Orquídea (entre rosa e roxo)
...
# Plotando linha para cada categoria (excluindo Total)
...
for i, categoria in enumerate(categorias):
    ...
    # Substituindo zeros por NaN para quebrar a linha
    valores_plotagem = qtde_ies.replace(0, float('nan'))
    
    # Plotando com NaN para quebrar linhas onde não há dados
...
# Configurando os ticks do eixo X para mostrar os anos como strings
