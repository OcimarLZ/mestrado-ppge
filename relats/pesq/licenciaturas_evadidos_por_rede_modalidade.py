...
select 
    ...
    sum(c.qt_sit_desvinculado + c.qt_sit_falecido + c.qt_sit_transferido) QtdeEvadidos  
from curso_censo c
...
pivot_df = df.pivot_table(index='ano', columns=['rede', 'modalidade'], values='QtdeEvadidos', fill_value=0).astype(int)
...
arq_nome = 'licenciaturas_evadidos_por_rede_modalidade'
...
ax.set_ylabel('Quantidade de Evadidos', fontsize=12)
ax.set_title('Evadidos em Licenciaturas por Tipo de Rede e Modalidade', color='#000000', fontsize=14)
