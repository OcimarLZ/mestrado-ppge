...
# Pivotando para ter colunas por rede
pivot_df = df.pivot_table(index='ano', columns='rede', values='QtdeMatriculas', fill_value=0).astype(int)
pivot_df.reset_index(inplace=True)

# Redefinindo a lista de tamanhos para o formato HTML
tam_colunas = ['50px'] + ['150px'] * (len(pivot_df.columns) - 1)
...
arq_nome = 'licenciaturas_matriculas_por_rede'
...
<table style=\"width: 100%; border-collapse: collapse;\">
    <tr style=\"background-color: #2E7D32;\">
        <th colspan=\"{len(pivot_df.columns)}\" style=\"font-size: 14px; font-family: Tahoma, sans-serif; color: white; padding: 10px; text-align: center;\">
            Matrículas em Licenciaturas por Tipo de Rede
        </th>
    </tr>
</table>
"""
...
ax.set_title('Matrículas em Licenciaturas por Tipo de Rede', color='#000000', fontsize=14)
ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.25), ncol=3, fontsize='medium', title='Tipo de Rede', frameon=False)
