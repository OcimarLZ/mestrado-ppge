...
# SQL parametrizado para recuperar municipios com polos UAB vinculados a alguma universidade federal.
sql = """
SELECT DISTINCT
    uc.ano_censo,
...
def gerar_mapa_combinado_a4(
    ...
    fig.suptitle(
        f'Municípios com Polos UAB em Universidades Federais: {ano_ini} x {ano_fim}',
        fontsize=13,
        fontweight='bold',
        y=0.97,
    )
...
def main() -> None:
    ...
    for ano in anos_alvo:
        ...
        print(f'Dados carregados para {ano}: {len(df_ano)} municipios (polos UAB em universidades federais)')

    anos_str = '_'.join(str(a) for a in anos_alvo)
    output_path = f'docs/graficos/mapa_municipios_polo_uab_uf_{anos_str}.png'
